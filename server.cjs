...
function ensureDb(){
  ...
  if(!fs.existsSync(DB_FILE)) writeDb({products:[], orders:[], accountingReports:[], notes:[], announcements:[], chatMembers:[], chatMessages:[], directMessages:[], presence:{}, readState:{}, carts:{}, sessions:{}, shops: defaultShops()});
}
function readDb(){ ensureDb(); try { const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); db.products=db.products||[]; db.orders=db.orders||[]; db.accountingReports=Array.isArray(db.accountingReports)?db.accountingReports:[]; db.notes=db.notes||[]; db.announcements=db.announcements||[]; db.chatMembers=Array.isArray(db.chatMembers)?db.chatMembers:[]; db.chatMessages=Array.isArray(db.chatMessages)?db.chatMessages:[]; db.directMessages=Array.isArray(db.directMessages)?db.directMessages:[]; db.presence=db.presence||{}; db.readState=db.readState||{}; db.carts=db.carts||{}; db.sessions=db.sessions||{}; normalizeShops(db); normalizeChat(db); sortProductsInCategories(db); return db; } catch(e){ return {products:[], orders:[], accountingReports:[], notes:[], announcements:[], chatMembers:[], chatMessages:[], directMessages:[], presence:{}, readState:{}, carts:{}, sessions:{}, shops: defaultShops()}; } }
...
function adminMenu(){ const msgUnread=adminDirectUnread(readDb()); return `<a class="mobileBackToCabinet" href="/admin">← Перейти у Кабінет складу</a><aside class="adminMenu"><div class="adminMenuHead"><a class="adminMenuLogo" href="/admin" style="text-decoration:none">Кабінет</a></div><a href="/admin-settings">Налаштування магазинів</a><a href="/admin-products">Товар</a><a data-unread-key="directMessages" href="/admin-messages">Повідомлення${badgeCount(msgUnread)}</a><a href="/admin-notes">Нотатки</a><a href="/admin-announcements">Оголошення</a><a href="/admin-chat">Чат</a><a href="/admin-orders">Замовлення</a><a href="/admin-accounting">Журнал обліку</a><a href="/admin-hidden-products">Приховані позиції</a><a href="/admin-logout" class="adminMenuLogout">Вийти</a></aside>`; }
...
function adminCabinetPage(db){
  const shops=getShops(db);
  const items=[['Налаштування магазинів','/admin-settings','⚙️'],['Товар','/admin-products','📦'],['Повідомлення','/admin-messages','✉️',adminDirectUnread(db)],['Нотатки','/admin-notes','📝'],['Оголошення','/admin-announcements','📢'],['Чат','/admin-chat','💬'],['Замовлення','/admin-orders','🧾'],['Журнал обліку','/admin-accounting','📒'],['Приховані позиції','/admin-hidden-products','🙈'],['Вийти зі складу','/admin-logout','🚪']];
...
function shopCabinetPage(db, session){
  ...
  const items=[['Каталог','/catalog','🛍️'],['Журнал обліку','/cabinet/accounting','📒'],['Кошик','/cart','🛒',(session.cart||[]).reduce((a,i)=>a+Number(i.qty||0),0)],['Повідомлення','/messages','✉️',unread.directMessages],['Оголошення','/about','📢',unread.announcements]];
...
function moneyNum(v){ const n=Number(String(v==null?'':v).replace(',','.')); return Number.isFinite(n)?Math.round(n*100)/100:0; }
function money(v){ return moneyNum(v).toFixed(2); }
...
function accountingTableRows(reports, admin=false){
  return reports.map((r,n)=>`<tr><td>${n+1}</td>${admin?`<td>${esc(r.shop||'')}</td>`:''}<td>${esc(r.date||'')}</td><td>${money(r.openingBalance)}</td><td>${money(r.fiscalReport)}</td><td>${money(r.terminalClose)}</td><td>${money(r.cash)}</td><td>${money(r.actualCash)}</td><td class="${Math.abs(moneyNum(r.discrepancy))>0.009?'warnText':''}">${money(r.discrepancy)}</td><td>${money(r.sentToOffice)}</td><td>${money(r.closingBalance)}</td><td>${esc(r.comment||'')}</td>${admin?`<td><div class="actions" style="gap:6px;flex-wrap:nowrap"><a class="btn secondary compactBtn" href="/admin-accounting-view?id=${encodeURIComponent(r.id)}">Перегляд</a><a class="btn secondary compactBtn" href="/admin-accounting-edit?id=${encodeURIComponent(r.id)}">Редагувати</a><form method="post" action="/admin-accounting-delete" onsubmit="return confirm('Видалити цей звіт?')"><input type="hidden" name="id" value="${esc(r.id)}"><button class="btn secondary compactBtn">Видалити</button></form></div></td>`:''}</tr>`).join('') || `<tr><td colspan="${admin?13:11}" class="center muted" style="padding:24px">Записів поки немає</td></tr>`;
}
function accountingPage(db, session){
  const reports=accountingRows(db).filter(r=>r.shop===session.shop).slice(0,60);
  const last=reports[0];
  const opening=last?money(last.closingBalance):'0.00';
  return `<section><div class="actions" style="align-items:center;justify-content:space-between;margin-bottom:12px"><h1 style="margin:0">Журнал обліку</h1><a class="btn secondary" href="/cabinet">До кабінету магазину</a></div><div class="card" style="padding:20px;margin-bottom:16px"><h2>Новий щоденний звіт</h2><p class="muted" style="margin-bottom:14px">Після збереження запис одразу зʼявиться в адмін-панелі складу. Кількість звітів за день не обмежена.</p><form class="form accountingForm" method="post" action="/accounting/save"><label>Дата<input type="date" name="date" value="${todayIsoWarsaw()}" required></label><label>Залишок на початок дня<input type="number" step="0.01" name="openingBalance" value="${opening}" required oninput="calcAccountingForm(this.form)"></label><label>Фіскальний рапорт<input type="number" step="0.01" name="fiscalReport" required oninput="calcAccountingForm(this.form)"></label><label>Закриття дня по терміналу<input type="number" step="0.01" name="terminalClose" required oninput="calcAccountingForm(this.form)"></label><label>Каса<input type="number" step="0.01" name="cash" readonly></label><label>Всього готівки в касі фактично<input type="number" step="0.01" name="actualCash" required oninput="calcAccountingForm(this.form)"></label><label>Розбіжність<input type="number" step="0.01" name="discrepancy" readonly></label><label>Передано в офіс<input type="number" step="0.01" name="sentToOffice" required oninput="calcAccountingForm(this.form)"></label><label>Залишок у касі<input type="number" step="0.01" name="closingBalance" readonly></label><label>Коментар<textarea name="comment" rows="3" placeholder="Необовʼязково"></textarea></label><button class="accountingSubmitBtn">Зберегти і надіслати</button></form></div><h2>Мої записи</h2><div class="listWrap"><table class="listTable accountingTable"><thead><tr><th>№</th><th>Дата</th><th>Залишок поч.</th><th>Фіскальний</th><th>Термінал</th><th>Каса</th><th>Факт.</th><th>Розбіжність</th><th>Офіс</th><th>Залишок</th><th>Коментар</th></tr></thead><tbody>${accountingTableRows(reports,false)}</tbody></table></div></section>`;
}
...
function adminAccountingEditPage(db, id){ const r=(db.accountingReports||[]).find(x=>String(x.id)===String(id)); if(!r) return `<div class="adminShell">${adminMenu()}<section class="card center"><h1>Запис не знайдено</h1><a class="btn" href="/admin-accounting">Назад</a></section></div>`; return `<div class="adminShell">${adminMenu()}<section><div class="actions" style="align-items:center;justify-content:space-between;margin-bottom:12px"><h1 style="margin:0">Редагування звіту</h1><a class="btn secondary" href="/admin-accounting">Назад</a></div><div class="card" style="padding:20px"><form class="form accountingForm" method="post" action="/admin-accounting-update"><input type="hidden" name="id" value="${esc(r.id)}"><label>Магазин<select name="shop" required>${getShops(db).map(s=>`<option value="${esc(s.name)}" ${s.name===r.shop?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label><label>Дата<input type="date" name="date" value="${esc(r.date||todayIsoWarsaw())}" required></label><label>Залишок на початок дня<input type="number" step="0.01" name="openingBalance" value="${money(r.openingBalance)}" required oninput="calcAccountingForm(this.form)"></label><label>Фіскальний рапорт<input type="number" step="0.01" name="fiscalReport" value="${money(r.fiscalReport)}" required oninput="calcAccountingForm(this.form)"></label><label>Закриття дня по терміналу<input type="number" step="0.01" name="terminalClose" value="${money(r.terminalClose)}" required oninput="calcAccountingForm(this.form)"></label><label>Каса<input type="number" step="0.01" name="cash" value="${money(r.cash)}" readonly></label><label>Всього готівки в касі фактично<input type="number" step="0.01" name="actualCash" value="${money(r.actualCash)}" required oninput="calcAccountingForm(this.form)"></label><label>Розбіжність<input type="number" step="0.01" name="discrepancy" value="${money(r.discrepancy)}" readonly></label><label>Передано в офіс<input type="number" step="0.01" name="sentToOffice" value="${money(r.sentToOffice)}" required oninput="calcAccountingForm(this.form)"></label><label>Залишок у касі<input type="number" step="0.01" name="closingBalance" value="${money(r.closingBalance)}" readonly></label><label>Коментар<textarea name="comment" rows="3">${esc(r.comment||'')}</textarea></label><button>Зберегти зміни</button></form></div></section></div>`; }
...
function accountingXlsx(reports){
  ...
  const sheetData=rows.map((row,ri)=>`<row r="${ri+1}">${row.map((v,ci)=>`<c r="${String.fromCharCode(65+ci)}${ri+1}" t="inlineStr" s="${ri===0?1:2}"><is><t>${xmlEsc(v)}</t></is></c>`).join('')}</row>`).join('');
...
function calcAccountingForm(form){function n(name){var el=form&&form.elements[name];var v=el?String(el.value||'').replace(',','.'):0;v=Number(v);return isFinite(v)?v:0}function set(name,val){var el=form&&form.elements[name];if(el)el.value=(Math.round(val*100)/100).toFixed(2)}var cash=n('fiscalReport')-n('terminalClose');set('cash',cash);set('discrepancy',n('actualCash')-n('openingBalance')-cash);set('closingBalance',n('actualCash')-n('sentToOffice'));}
...
<div class="layout2">
  ...
  if(req.method==='POST' && url.pathname==='/admin-login'){ const b=await body(req); if(String(b.password)===ADMIN_PASSWORD){ session.admin=true; saveSession(session); return redirect(res,'/admin'); } return send(res, layout('Вхід у склад', adminLoginPage('Невірний пароль. Спробуйте ще раз.'), session), 401); }
